$(document).ready(function () {
  ...
  if (app.isPage('/book-appointment')) {
...
var app = {
  ...
  getInstagramPhotos: function (options) {
    ...
    .done(function (media) {
      $.each(media.data, function (index, photo) {
        ...
        if (title.length > maxLength) {
          var subString = title.substr(0, maxLength - 1);
        ...
        if (photo.type === 'image') {
          ...
          html += '<a href="' + photo.videos.standard_resolution.url + '" type="video/mp4"' +
            ' data-poster="' + img.url + '"';
        }
    ...
    .fail(function () {
      var html = '<div class="alert alert-danger">Failed to load Instagram photos</div>';
      $container.html(html);
    });
